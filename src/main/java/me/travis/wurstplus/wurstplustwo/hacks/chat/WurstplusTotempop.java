        ...
        if (event.get_packet() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35) {
                ...
                if (WurstplusFriendUtil.isFriend(entity.getName())) {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " TotemPop " + reset + grey + " > " + reset + "ALERT, " + bold + green + entity.getName() + reset + " has popped " + bold + count + reset + " totems.");
                } else {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " TotemPop " + reset + grey + " > " + reset + "ALERT, " + bold + red + entity.getName() + reset + " has popped " + bold + count + reset + " totems.");
                }
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            ...
            if (player.isDead || player.getHealth() <= 0) {
                ...
                if (WurstplusFriendUtil.isFriend(player.getName())) {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " TotemPop " + reset + grey + " > " + reset + "ALERT, " + bold + green + player.getName() + reset + " just DIED after popping " + bold + count + reset + " totems.");
                } else {
                    WurstplusMessageUtil.send_client_message( red + "" + bold + " TotemPop " + reset + grey + " > " + reset + "ALERT, " + bold + red + player.getName() + reset + " just DIED after popping " + bold + count + reset + " totems");
                }
